...
#include "Hardware_Hidden_Symulink_private.h"
#include "rtwtypes.h"
#include "limits.h"
...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    if (terminatingmodel)
      break;
    Hardware_Hidden_Symulink_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Hardware_Hidden_Symulink_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    if (rtmStepTask(Hardware_Hidden_Symulink_M, 1)
        ) {
      mw_osSignalEventRelease(&subrateTaskSem[0]);
    }

    Hardware_Hidden_Symulink_step(0);

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Hardware_Hidden_Symulink_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Hardware_Hidden_Symulink_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  Hardware_Hidden_Symulink_terminate();
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Hardware_Hidden_Symulink_M, 0);

  /* Initialize model */
  Hardware_Hidden_Symulink_initialize();
