...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(pacemakerA1_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    pacemakerA1_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(pacemakerA1_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(pacemakerA1_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(pacemakerA1_M, 0);

  /* Initialize model */
  pacemakerA1_initialize();
